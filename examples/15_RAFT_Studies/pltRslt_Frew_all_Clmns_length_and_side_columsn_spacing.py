...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(run_dir, "outputs/15_RAFT_optimization_freq_step1_allColumns_length_and_side_columns_spacing") #opt coByla
    ...
    #-------------------------------------Objectives------------------------------------
    pltfrm_mass_vec=np.squeeze(outdata[0]['raft.platform_mass'])*1e-6
    ...
    blade_mass_vec=np.squeeze(outdata[0]['rotorse.blade_mass'])*1e-6
    tower_mass_vec=np.squeeze(outdata[0]['towerse.tower_mass'])*1e-6
    ...
    # Step 1: Find unique values and their first occurrence indices
    unique_values, unique_indices = np.unique(Clmns_Length_vec, return_index=True)
    indices_dict = {}
    for value in unique_values:
        indices = np.where(Clmns_Length_vec == value)[0]
        indices_dict[value] = indices
    #Max_PtfmPitch_vc[indices_dict[35.0]]
    ...
    #Y_pntoon_upper_vec_mesh=Y_pntoon_upper_vec.reshape((8,8,8))
    #Y_pntoon_lower_vec_mesh=Y_pntoon_lower_vec.reshape((8,8,8))
    Max_PtfmPitch_vc_mesh=Max_PtfmPitch_vc.reshape((8,8,8))
    Std_PtfmPitch_vec_mesh=Std_PtfmPitch_vec.reshape((8,8,8))
    pltfrm_mass_vec_mesh=pltfrm_mass_vec.reshape((8,8,8))
    AEP_vec_mesh=AEP_vec.reshape((8,8,8))
    cost_vec_mesh=cost_vec.reshape((8,8,8))
    finance_lcoe_vec_mesh=finance_lcoe_vec.reshape((8,8,8))
    floatingse_structurall_mass_mesh=floatingse_structurall_mass.reshape((8,8,8))
    main_column_mass_mesh=main_column_mass.reshape((8,8,8))
    side_columns_mass_mesh=side_columns_mass.reshape((8,8,8))
    ...
    surf = ax.scatter(Clmns_keel_vec_mesh.flatten(), Clmns_freeboard_vec_mesh.flatten(), Clmns_spacing_vec_mesh.flatten(), c=Max_PtfmPitch_vc_mesh.flatten(), cmap='Reds')
    fig.colorbar(surf, ax=ax, label='Max_PtfmPitch_vc_mesh')
    ax.set_xlabel('Clmns_keel_vec_mesh')
    ax.set_ylabel('Clmns_freeboard_vec_mesh')
    ...
    for ii in np.arange(8):
        ...
        ax.set_xticklabels([f'{Clmns_keel_vec_mesh[ii, 0, i]:.2f}' for i in x_indices])
        ...
        ax.set_yticklabels([f'{Clmns_freeboard_vec_mesh[ii, i, 0]:.1f}' for i in y_indices])
        ax.set_xlabel('$Z_\mathrm{Keel}$', fontsize=25)
        ax.set_ylabel('$Z_\mathrm{frbrd}$', fontsize=25)
        ax.set_title(r'$\mathrm{Max\,\,Ptfm\,\,Pitch\,\,[deg]}$' + f' - Clmn Spc : {Clmns_spacing_vec_mesh[ii][0][0]}', fontsize=20)
        ...
        for i in range(Max_PtfmPitch_vc_mesh[ii].shape[0]):
            for j in range(Max_PtfmPitch_vc_mesh[ii].shape[1]):
                ax.text(j, i, f'{Max_PtfmPitch_vc_mesh[ii][i, j]:.1f}', ha='center', va='center', color='black', fontsize=20)
        if Clmns_spacing_vec_mesh[ii][0][0] == 51.57:  # baseline
            ax.plot(np.where(Clmns_keel_vec_mesh[ii, 0, :] == -20)[0][0], np.where(Clmns_freeboard_vec_mesh[ii, :, 0] == 15)[0][0], 's', markersize=35, markerfacecolor='none', markeredgecolor='black', markeredgewidth=3)
        fig.savefig(os.getcwd() + '/outputs/' + f'Max_PtfmPitch_vc_mesh_all_clmns_legtn_and_spacing_{ii}.jpg', format='jpg', dpi=300, bbox_inches='tight')
    ...
    for ii in np.arange(8):
        ...
        ax.set_xlabel('$Z_\mathrm{Keel}$', fontsize=25)
        ax.set_ylabel('$Z_\mathrm{frbrd}$', fontsize=25)
        ax.set_title(r'$\mathrm{Std\,\,Ptfm\,\,Pitch\,\,[deg]}$' + f' - Clmn Spc : {Clmns_spacing_vec_mesh[ii][0][0]}', fontsize=20)
        # Customize the colorbar ticks
        cbar = plt.colorbar(ScalarMappable(norm=norm, cmap='Reds'), ax=ax)
        # Annotate each element with its value
        for i in range(Std_PtfmPitch_vec_mesh[ii].shape[0]):
            for j in range(Std_PtfmPitch_vec_mesh[ii].shape[1]):
                ax.text(j, i, f'{Std_PtfmPitch_vec_mesh[ii][i, j]:.1f}', ha='center', va='center', color='black', fontsize=20)
        if Clmns_spacing_vec_mesh[ii][0][0] == 51.57:  # baseline
            ax.plot(np.where(Clmns_keel_vec_mesh[ii, 0, :] == -20)[0][0], np.where(Clmns_freeboard_vec_mesh[ii, :, 0] == 15)[0][0], 's', markersize=35, markerfacecolor='none', markeredgecolor='black', markeredgewidth=3)
        fig.savefig(os.getcwd() + '/outputs/' + f'Std_PtfmPitch_vec_mesh_all_clmns_legtn_and_spacing_{ii}.jpg', format='jpg', dpi=300, bbox_inches='tight')
        #plt.show()  # Show the plot
        plt.close(fig)
    plt.close('all')
    ...
    for ii in np.arange(8):
        ...
        ax.set_xlabel('$Z_\mathrm{Keel}$', fontsize=25)
        ax.set_ylabel('$Z_\mathrm{frbrd}$', fontsize=25)
        ax.set_title(r'$m_p \,\, \mathrm{floatingse - structurall}$' + f' - Clmn Spc : {Clmns_spacing_vec_mesh[ii][0][0]}', fontsize=20)
        # Customize the colorbar ticks
        cbar = plt.colorbar(ScalarMappable(norm=norm, cmap='Reds'), ax=ax)
        # Annotate each element with its value
        for i in range(floatingse_structurall_mass_mesh[ii].shape[0]):
            for j in range(floatingse_structurall_mass_mesh[ii].shape[1]):
                ax.text(j, i, f'{floatingse_structurall_mass_mesh[ii][i, j]:.2f}', ha='center', va='center', color='black', fontsize=20)
        if Clmns_spacing_vec_mesh[ii][0][0] == 51.57:  # baseline
            ax.plot(np.where(Clmns_keel_vec_mesh[ii, 0, :] == -20)[0][0], np.where(Clmns_freeboard_vec_mesh[ii, :, 0] == 15)[0][0], 's', markersize=35, markerfacecolor='none', markeredgecolor='black', markeredgewidth=3)
        fig.savefig(os.getcwd() + '/outputs/' + f'floatingse_structurall_mass_mesh_all_clmns_legtn_and_spacing_{ii}.jpg', format='jpg', dpi=300, bbox_inches='tight')
        #plt.show()  # Show the plot
        plt.close(fig)
    plt.close('all')
    ...
    for ii in np.arange(8):
        ...
        ax.set_xlabel('$Z_\mathrm{Keel}$', fontsize=25)
        ax.set_ylabel('$Z_\mathrm{frbrd}$', fontsize=25)
        ax.set_title(r'$m_p \,\, \mathrm{floatingse - platfrom}$' + f' - Clmn Spc : {Clmns_spacing_vec_mesh[ii][0][0]}', fontsize=20)
        # Customize the colorbar ticks
        cbar = plt.colorbar(ScalarMappable(norm=norm, cmap='Reds'), ax=ax)
        # Annotate each element with its value
        for i in range(floatingse_platform_mass_vec_mesh[ii].shape[0]):
            for j in range(floatingse_platform_mass_vec_mesh[ii].shape[1]):
                ax.text(j, i, f'{floatingse_platform_mass_vec_mesh[ii][i, j]:.2f}', ha='center', va='center', color='black', fontsize=20)
        if Clmns_spacing_vec_mesh[ii][0][0] == 51.57:  # baseline
            ax.plot(np.where(Clmns_keel_vec_mesh[ii, 0, :] == -20)[0][0], np.where(Clmns_freeboard_vec_mesh[ii, :, 0] == 15)[0][0], 's', markersize=35, markerfacecolor='none', markeredgecolor='black', markeredgewidth=3)
        fig.savefig(os.getcwd() + '/outputs/' + f'floatingse_platform_mass_vec_mesh_all_clmns_legtn_and_spacing_{ii}.jpg', format='jpg', dpi=300, bbox_inches='tight')
        #plt.show()  # Show the plot
        plt.close(fig)
    plt.close('all')
    #--------------------------------- AEP_vec_mesh -----------------------------------------
    ...
    for ii in np.arange(8):
        ...
        ax.set_yticks(y_indices)
        ax.set_yticklabels([f'{Clmns_freeboard_vec_mesh[ii, i, 0]:.1f}' for i in y_indices])
        ax.set_ylabel('$Z_\mathrm{Keel}$', fontsize=15)
        ax.set_xlabel('$Z_\mathrm{frbrd}$', fontsize=15)
        ax.set_title('$AEP$', fontsize=15)
        ax.set_title(r'$\mathrm{AEP}$' + f' - Clmn Spc : {Clmns_spacing_vec_mesh[ii][0][0]}', fontsize=15)
        # Customize the colorbar ticks
        cbar = plt.colorbar(ScalarMappable(norm=norm, cmap='Reds'), ax=ax)
        # Annotate each element with its value
        for i in range(AEP_vec_mesh[ii].shape[0]):
            for j in range(AEP_vec_mesh[ii].shape[1]):
                ax.text(j, i, f'{AEP_vec_mesh[ii][i, j]*1e-6:.1f}', ha='center', va='center', color='black', fontsize=8)
        if Clmns_spacing_vec_mesh[ii][0][0] == 51.57:  # baseline
            ax.plot(np.where(Clmns_keel_vec_mesh[ii, 0, :] == -20)[0][0], np.where(Clmns_freeboard_vec_mesh[ii, :, 0] == 15)[0][0], 's', markersize=20, markerfacecolor='none', markeredgecolor='black', markeredgewidth=3)
        fig.savefig(os.getcwd() + '/outputs/' + f'AEP_vec_mesh_all_clmns_legtn_and_spacing_{ii}.jpg', format='jpg', dpi=300, bbox_inches='tight')
        #plt.show()  # Show the plot
        """