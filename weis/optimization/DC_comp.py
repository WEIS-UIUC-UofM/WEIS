...
model.add_subsystem('model', subsys=SM_Comp(), promotes_inputs=opt_dv_key_wo_dot, promotes_outputs=opt_output_key_wo_dot)
...
for k in range(len(opt_params_key)):  #need to get the values from Saeid's code, the sm inputs that are not dvs or opt parameters also need vals
    ...
    if opt_params_key[k] == 'floating.jointdv_0':
        model.set_input_defaults('floating_jointdv_0', val=6778.0)
        
    elif opt_params_key[k]  == "floating.jointdv_1":
        model.set_input_defaults('floating_jointdv_1', val=6778.0)
        
    elif opt_params_key[k]  == "floating.jointdv_2":
        model.set_input_defaults('floating_jointdv_2', val=6778.0)
        
    elif opt_params_key[k]  == "floating.memgrp0.outer_diameter_in":
        model.set_input_defaults('floating_memgrp0_outer_diameter_in', val=6778.0)
        
    elif opt_params_key[k]  == "floating.memgrp1.outer_diameter_in":
        model.set_input_defaults('floating_memgrp1_outer_diameter_in', val=6778.0)
        
    elif opt_params_key[k]  == "floating.memgrp2.outer_diameter_in":
        model.set_input_defaults('floating_memgrp2_outer_diameter_in', val=6778.0)
        
    elif opt_params_key[k]  == "floating.memgrp3.outer_diameter_in":
        model.set_input_defaults('floating_memgrp3_outer_diameter_in', val=6778.0)
        
    elif opt_params_key[k]  == "tune_rosco_ivc.ps_percent":
        model.set_input_defaults('tune_rosco_ivc_ps_percent', val=6778.0)
        
    elif opt_params_key[k]  == "tune_rosco_ivc.omega_pc":
        model.set_input_defaults('tune_rosco_ivc_omega_pc', val=6778.0)
        
    elif opt_params_key[k]  == "tune_rosco_ivc.ptfm_freq":
        model.set_input_defaults('tune_rosco_ivc_ptfm_freq', val=6778.0)
        
    elif opt_params_key[k]  == "tune_rosco_ivc.zeta_pc":
        model.set_input_defaults('tune_rosco_ivc_zeta_pc', val=6778.0)
        
    elif opt_params_key[k]  == "configuration.rotor_diameter_user":
        model.set_input_defaults('configuration_rotor_diameter_user', val=6778.0)
    
    elif opt_params_key[k]  == "floating.member_main_column:outer_diameter":
        model.set_input_defaults('floating_member_main_column:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_column1:outer_diameter":
        model.set_input_defaults('floating_member_column1:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_column2:outer_diameter":
        model.set_input_defaults('floating_member_column2:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_column3:outer_diameter":
        model.set_input_defaults('floating_member_column3:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_Y_pontoon_upper1:outer_diameter":
        model.set_input_defaults('floating_member_Y_pontoon_upper1:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_Y_pontoon_upper2:outer_diameter":
        model.set_input_defaults('floating_member_Y_pontoon_upper2:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_Y_pontoon_upper3:outer_diameter":
        model.set_input_defaults('floating_member_Y_pontoon_upper3:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_Y_pontoon_lower1:outer_diameter":
        model.set_input_defaults('floating_member_Y_pontoon_lower1:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_Y_pontoon_lower2:outer_diameter":
        model.set_input_defaults('floating_member_Y_pontoon_lower2:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "floating.member_Y_pontoon_lower3:outer_diameter":
        model.set_input_defaults('floating_member_Y_pontoon_lower3:outer_diameter', val=6778.0)
        
    elif opt_params_key[k]  == "tune_rosco_ivc.Kp_float":
        model.set_input_defaults('tune_rosco_ivc_Kp_float', val=6778.0)
...
for k in range(len(constraints_key)):  #need to get the lower and upper values from Saeid's code
    ...
    if constraints_key[k] == 'raft.Max_PtfmPitch':
        prob.model.add_constraint('raft_Max_PtfmPitch', lower = 0, upper=2.5)
        
    elif constraints_key[k]  == "raft.Std_PtfmPitch":
        prob.model.add_constraint('raft_Std_PtfmPitch', lower = 0, upper=2.5)
        
    elif constraints_key[k]  == "raft.max_nac_accel":
        prob.model.add_constraint('raft_max_nac_accel', lower = 0, upper=2.5)
        
    elif constraints_key[k]  == "floatingse.structural_frequencies":
        prob.model.add_constraint('floatingse_structural_frequencies', lower = 0, upper=2.5)

    else:
        print("Please choose correct constraint")


prob.model.add_objective(objective_key_wo_dot[0])
...
def save_opt_output(prob):
    objective_st = prob.get_val(objective_key_wo_dot[0])  
    
    # output dictionary
    opt_output = {
      "dvs": {"dv_keys": [], "dv_values":np.zeros((len(opt_dv_key),))},
      "objective": {"objective_keys": objective_key, "objective_values":objective_st},
      "constraints":{"constraints_keys": [], "constraints_values":np.zeros((len(constraints_key),))}, 
      # "exit_flag":
    }
        
    for k in range(len(constraints_key)):
        cons_val = prob.get_val(constraints_key_wo_dot[k]) 
        opt_output['constraints']['constraints_keys'].append(constraints_key[k])
        opt_output['constraints']['constraints_values'][k] = cons_val
        
    for k in range(len(opt_dv_key)):
        dv_val = prob.get_val(opt_dv_key_wo_dot[k]) 
        opt_output['dvs']['dv_keys'].append(opt_dv_key[k])
        opt_output['dvs']['dv_values'][k] = dv_val
    return opt_output

opt_output = save_opt_output(prob)