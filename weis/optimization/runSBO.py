...
class WindTurbineSMOpt():
    ...
    def create_problem(self):
        ...
        self.prob = prob
        model = prob.model
        model.add_subsystem('model', subsys=SM_Comp(self), promotes_inputs=self.opt_input_key_wo_dot, promotes_outputs=self.opt_output_key_wo_dot+['raft_heave_avg']+['raft_Std_PtfmPitch']+['raft_heave_period']+['raft_pitch_period'])
    ...
    def run_optimization(self, DCA_req):
        ...
        prob = self.prob
        prob.model.add_objective(self.objective_key_wo_dot[0], ref=1e3)
...
class SM_Comp(om.ExplicitComponent):
    ...
    def __init__(self, WindTurbineSMOpt):
        super().__init__()
        self.opt_dv_key = WindTurbineSMOpt.opt_dv_key
    ...
    def setup(self):
        ...
        opt_dv_key = self.opt_dv_key
        # sm_params = list(set(avail_input_keys).difference(opt_dv_key))
        for k in range(len(avail_input_keys)):   #add all sm inputs, add their values. Need to get x0 in a better way
            
            if avail_input_keys[k] == 'floating.jointdv_0':
                self.add_input('floating_jointdv_0', -20.)
                
            elif avail_input_keys[k]  == "floating.jointdv_1":
                self.add_input('floating_jointdv_1',15.)
                
            elif avail_input_keys[k]  == "floating.jointdv_2":
                self.add_input('floating_jointdv_2',51.75)
                
            elif avail_input_keys[k]  == "floating.memgrp0.outer_diameter_in":
                self.add_input('floating_memgrp0_outer_diameter_in',10.)
                
            elif avail_input_keys[k]  == "floating.memgrp1.outer_diameter_in":
                self.add_input('floating_memgrp1_outer_diameter_in',12.5)
                
            elif avail_input_keys[k]  == "floating.memgrp2.outer_diameter_in":
                self.add_input('floating_memgrp2_outer_diameter_in',0.92)
                
            elif avail_input_keys[k]  == "floating.memgrp3.outer_diameter_in":
                self.add_input('floating_memgrp3_outer_diameter_in',9.6148)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.ps_percent":
                self.add_input('tune_rosco_ivc_ps_percent',0.85)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.omega_pc":
                self.add_input('tune_rosco_ivc_omega_pc',0.2)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.ptfm_freq":
                self.add_input('tune_rosco_ivc_ptfm_freq',0.2)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.zeta_pc":
                self.add_input('tune_rosco_ivc_zeta_pc',1.)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.omega_vs": 
                self.add_input('tune_rosco_ivc_omega_vs',0.12)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.zeta_vs": 
                self.add_input('tune_rosco_ivc_zeta_vs',0.85)
                
            elif avail_input_keys[k]  == "configuration.rotor_diameter_user":
                self.add_input('configuration_rotor_diameter_user',241.94)
                
            elif avail_input_keys[k]  == "floating.member_main_column:outer_diameter":
                self.add_input('floating_member_main_column:outer_diameter',10.)
                
            elif avail_input_keys[k]  == "floating.member_column1:outer_diameter":
                self.add_input('floating_member_column1:outer_diameter',12.5)
                
            elif avail_input_keys[k]  == "floating.member_column2:outer_diameter":
                self.add_input('floating_member_column2:outer_diameter', 12.5)
                
            elif avail_input_keys[k]  == "floating.member_column3:outer_diameter":
                self.add_input('floating_member_column3:outer_diameter',12.5)
                
            elif avail_input_keys[k]  == "floating.member_Y_pontoon_upper1:outer_diameter":
                self.add_input('floating_member_Y_pontoon_upper1:outer_diameter',0.91)
                ...
                self.add_input('floating_member_Y_pontoon_lower1:outer_diameter',9.6148)
                ...
                self.add_input('tune_rosco_ivc_Kp_float',-10.)
            else:
        ...
        for k in range(len(opt_output_key)):
            ...
            elif opt_output_key[k]  == "raft.heave_avg":
                self.add_output('raft_heave_avg')
            elif opt_output_key[k]  == "raft.Std_PtfmPitch":
                self.add_output('raft_Std_PtfmPitch')
            elif opt_output_key[k]  == "raft.heave_period":
                self.add_output('raft_heave_period')
            elif opt_output_key[k]  == "raft.pitch_period":
                self.add_output('raft_pitch_period')
    ...
    def compute(self, inputs, outputs):
        ...
        for k in range(len(avail_input_keys)):   #add all sm inputs
            ...
            if avail_input_keys[k] == 'floating.jointdv_0':
                ...
                tune_rosco_ivc_zeta_vs = inputs['tune_rosco_ivc_zeta_vs']
                predict_input.append(tune_rosco_ivc_zeta_vs)
                
            elif avail_input_keys[k]  == "tune_rosco_ivc.omega_vs":
                tune_rosco_ivc_omega_vs = inputs['tune_rosco_ivc_omega_vs']
                predict_input.append(tune_rosco_ivc_omega_vs)
        ...
        for k in range(len(opt_output_key)):
            ...
            if opt_output_key[k] == 'raft.Max_PtfmPitch':
                ...
                outputs['raft_heave_avg'] = self.run_predict(['raft.heave_avg'], predict_input_flat)
                
            elif opt_output_key[k]  == "raft.Std_PtfmPitch":
                outputs['raft_Std_PtfmPitch'] = self.run_predict(['raft.Std_PtfmPitch'], predict_input_flat)
            
            elif opt_output_key[k]  == "raft.heave_period":
                outputs['raft_heave_period'] = self.run_predict(['raft.heave_period'], predict_input_flat)
            
            elif opt_output_key[k]  == "raft.pitch_period":
                outputs['raft_pitch_period'] = self.run_predict(['raft.pitch_period'], predict_input_flat)
